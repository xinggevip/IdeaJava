...
public class UploadDownController {
    ...
    @ResponseBody
    public Result uploadheadpicture(@RequestParam("userPicture") MultipartFile albumPicture, HttpServletRequest request) {
        ...
        try {
            ...
            System.out.println("上传成功");
            //将文件在服务器的存储路径返回
            return new Result(true,"/headpicture/" + fileName,100);
        } catch (IOException e) {
            System.out.println("上传失败");
            e.printStackTrace();
            return new Result(false, "上传失败",-100);
    ...
    public Result edituser(@RequestBody Users user) {
        Result result = usersService.updataUser(user);
        return result;
    }

    @RequestMapping("/deluserpicture")
    @ResponseBody
    public Result deluserpicture(@RequestBody Users user, HttpServletRequest request){
        System.out.println("来到了delpicture");
        System.out.println(user.getUserPicture());

        //获取文件在服务器的储存位置
        String path = request.getSession().getServletContext().getRealPath(user.getUserPicture());
        System.out.println("文件在服务器的储存位置:=====" + path);
        ...
        if (!filePath.exists() && !filePath.isDirectory()) {
            ...
            // 存在则删除并返回信息
            System.out.println("存在文件，开始删头像吧");
            boolean delete = filePath.delete();
            if (delete == true){
                return new Result(true,"删除成功");
            }else{
